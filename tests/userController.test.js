...
describe('POST /api/users/send-friend-request/:userId', () => {
  ...
  it('should send a friend request from the current user to the specified user', async () => {
    ...
    expect(res.body.currentUser.friendRequestsSent).toContainEqual(userToSend.id);
...
describe('POST /api/users/accept-friend-request/:userId', () => {
  ...
  beforeEach(async () => {
    ...
    await testSession
      ...
      await testSession
        .post(`/api/users/send-friend-request/${user.id}`);
  ...
  it('should accept friend request from the user', async () => {
    ...
    expect(res.body.user.friends)
    .toContainEqual(userFriendRequestFrom.id)
    expect(res.body.user.friendRequestsReceived)
    .not.toContainEqual(userFriendRequestFrom.id)
    expect(res.body.otherUser.friends)
    .toContainEqual(user.id)
    expect(res.body.otherUser.friendRequestsSent)
    .not.toContainEqual(user.id);
  });
