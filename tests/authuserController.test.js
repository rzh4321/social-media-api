...
describe("GET /api/authuser/posts", () => {
  ...
  it("should return 401 error if user is not logged in", async () => {
    await testSession.post("/api/auth/logout").expect(200);
    const res = await testSession.get("/api/authuser/posts").expect(401);
    expect(res.body.message).toBe("Unauthorized");
  });
  ...
  it("should return 404 error if the user is not found in database", async () => {
    ...
    const res = await testSession.get("/api/authuser/posts").expect(404);
    expect(res.body.message).toBe("User not found");
    // Add Bob back to the database
    const signUpRes = await request(app)
      .post("/api/auth/signup")
  ...
  it("should return a list of posts", async () => {
    ...
    await testSession.post("/api/authuser/posts").send(mockPost).expect(201);
    let res = await testSession.get("/api/authuser/posts").expect(200);
    ...
    await testSession
      ...
      .expect(201);
    res = await testSession.get("/api/authuser/posts").expect(200);
...
describe("POST /api/authuser/posts/:postid/give-like", () => {
  ...
  it("should add the user to the posts likes array", async () => {
    const res = await testSession
      .post(`/api/authuser/posts/${mockPost.id}/give-like`)
      .expect(200);
    expect(res.body.message).toBe("Like added");
    expect(res.body.post.likes).toContainEqual(user.id.toString());
  });

  it("should return a 400 error if the post is already liked by the user", async () => {
